...
public class EmployeeController {
	...
	public EditEmployeeResponse editEmployee(@PathVariable("id") int id, @Valid @RequestBody Employee employee) {
		 ...
		 * validationEmployeeUtil.validateEditEmployeeRequest(id); if
	...
	public GetEmployeeResponse getEmployeeById(@PathVariable("id") int id) {
		Set<String> errorMessages = validationEmployeeUtil.validateGetEmployeeRequest(id);
		if (!CollectionUtils.isEmpty(errorMessages)) {
			return new GetEmployeeResponse(HttpStatus.PRECONDITION_FAILED, "999", errorMessages);
		} else {
